	...
	while(true){
		...
		if (System::Time()!=time_img){
			...
			if (cam->IsAvailable()&&time_img%50==0){
				...
				int left_corner_sum=0;
				int right_corner_sum=0;
				...
				bool do_edge_find=true;
				const int left_base=left_edge[0][0],right_base=right_edge[0][0];

				if(get_pkbit(byte,left_edge[0][0],left_edge[0][1]))
					while(get_pkbit(byte,++left_edge[0][0],left_edge[0][1])){
						/*if(left_edge[0][0]>WIDTH){
							left_edge[0][0]=left_base;
							do_edge_find=false;
						}*/
					}
				else
				...
				for (int count=0;count<400-1;count++){
					...
					if(do_feature_extract){
						if(left_capture_time==0&&left_edge[left_sum-1][1]<HEIGHT/2){
							if(left_edge[left_sum-1][0]>4){
								left_thershold=left_edge[left_sum-1][0];
								left_capture_time=1;
								lcd->SetRegion(Lcd::Rect(left_edge[left_sum-1][0],HEIGHT-left_edge[left_sum-1][1]-1,5,5));
								lcd->FillColor(Lcd::kRed);
							}
						}
						else if(left_capture_time==1){
							if(left_edge[left_sum-1][0]<left_thershold){
								left_capture_time=2;
								lcd->SetRegion(Lcd::Rect(left_edge[left_sum-1][0],HEIGHT-left_edge[left_sum-1][1]-1,5,5));
								lcd->FillColor(Lcd::kRed);
...
led1.SetEnable(1);
					...
					//check if do feature extraction
					if(do_feature_extract){
						if(right_capture_time==0&&right_edge[right_sum-1][1]<HEIGHT/2){
							if(right_edge[right_sum-1][0]<WIDTH-1-4){
								right_thershold=right_edge[right_sum-1][0];
								right_capture_time=1;
								lcd->SetRegion(Lcd::Rect(right_edge[right_sum-1][0],HEIGHT-right_edge[right_sum-1][1]-1,5,5));
								lcd->FillColor(Lcd::kPurple);
							}
						}
						else if(right_capture_time==1){
							if(right_edge[right_sum-1][0]>right_thershold){
				...
				if(left_capture_time==1&&right_capture_time==1){
					print_feature("straight",&writer);
				}
				else if(left_capture_time>1&&right_capture_time==1){
					print_feature("left_turn",&writer);
				}
				else if(left_capture_time==1&&right_capture_time==0){
					print_feature("right_turn",&writer);
				}
				else if(left_capture_time==0&&right_capture_time==1){
					print_feature("steep_left",&writer);
				}
				else if(left_capture_time==1&&right_capture_time==0){
					print_feature("steep_right",&writer);
				}
				else{
					print_feature("+ or O",&writer);
				}
